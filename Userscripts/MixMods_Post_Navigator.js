...
// @homepageURL         https://github.com/ChaliceChore/JSScripts/tree/main/Userscripts
// @supportURL          https://github.com/ChaliceChore/JSScripts/issues
// @downloadURL         https://raw.githubusercontent.com/ChaliceChore/JSScripts/refs/heads/main/Userscripts/MixMods_Post_Navigator.js
// @updateURL           https://raw.githubusercontent.com/ChaliceChore/JSScripts/refs/heads/main/Userscripts/MixMods_Post_Navigator.js
// ==/UserScript==
...
(() => {
    ...
    /* Ignore individual post pages use the date-based permalink /YYYY/MM/slug/.
       Everything else (homepage, category/tag listings, /page/N/, etc.) is a
       listing page and should get the nav overlay. */
    ...
    /* Keep `posts` in sync when infinite-scroll userscripts (e.g. Pagetual)
       append the next page's markup into the DOM. `posts` is mutated in place
       (not reassigned) so every closure below that reads posts.at(0),
    ...
    /* Infinite-scroll userscripts like Pagetual fetch the next /page/N/ and
       ...
       navigation, so nothing else here would notice new posts showed up. Watch
       that container and re-scan whenever content is added. Debounced, since
       these appends are often several DOM mutations in a row. */
    ...
    /* The site's native language is Portuguese; the GTranslate widget lets
       ...
       For any other explicit selection (including Portuguese), we leave
       translation enabled so GTranslate keeps the labels in sync with the rest
       of the page. */
    const detectLanguage = () => {
        ...
        if (wrapper) {
            ...
            selectedValue =
                (selected?.value || selected?.textContent || "").toLowerCase();
        }
        ...
        // ── 2. Fallback ──────────────────────────────────────────────────────
        /* No explicit English selection (a different language is selected, or
           GTranslate hasn't reported a selection at all) — pick a baseline via
           the HTML lang attribute, then the GTranslate value itself, defaulting
           to English, and allow translation to proceed. */
    ...
    /* Builds the floating action buttons / mobile FAB, injects their styling,
       and wires up all of their interaction behavior (long-press tooltips,
       mobile expand/collapse, and following the site's theme toggle).
       Confirm-state tooltips are pre-rendered and switched purely via CSS (see
       the "mm-tip-default"/"mm-tip-confirm" rules) rather than rewritten at
       runtime, so GTranslate's translation of them survives. */
    const createFloatingNav = () => {
        ...
        style.textContent = /* css */`
            ...
            .mm-nav button:hover {
                ...
                color: #453aa8;
                box-shadow:
                    0 8px 20px rgba(91, 79, 196, 0.25),
        ...
        buttonConfigs.forEach((config) => {
            ...
            /* Both tooltip variants are rendered up front and switched purely
               via CSS (the "mm-confirm" class). This matters for GTranslate:
               it only translates text that's present in the DOM at scan time —
               if we instead rewrote .textContent on click (as before), the
               freshly-injected text would show up untranslated (falling back to
               the script's own PT/EN baseline) instead of staying in the
